...
public class CatalogPage {
   ...
   private boolean initOtherData() {
      DatabaseResult GetHeaders = Grizzly.getStorage().preparedQuery("SELECT * FROM server_store_page_headers WHERE page_id = ?", Arrays.asList(new String[]{String.valueOf(this.ID)}));
      ...
      DatabaseResult GetTexts = Grizzly.getStorage().preparedQuery("SELECT * FROM server_store_page_texts WHERE page_id = ?", Arrays.asList(new String[]{String.valueOf(this.ID)}));
