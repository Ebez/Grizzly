...
import net.cobem.grizzly.habbohotel.pathfinding.ghost.Ghost_PathfinderValues;

public class Ghost_PathfinderLogic implements IPathfinder {
   ...
   public Collection<byte[]> Path(byte startX, byte startY, byte endX, byte endY, float maximumFall, float maximumJump) {
      ...
      Ghost_PathfinderValues values;
      synchronized(this.collisionMap) {
         values = new Ghost_PathfinderValues(this.collisionMap, this.height, maximumFall, maximumJump);
   ...
   private void Add(int x, int y, byte endX, byte endY, Ghost_PathfinderValues values) {
   ...
   private static void Move(Ghost_PathfinderValues values) {
